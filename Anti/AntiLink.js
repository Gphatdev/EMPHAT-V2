...
// Lấy nội dung text an toàn, tránh crash khi content là object (vd: chat.recommended)
function getSafeMsgBody(rawContent) {
  if (typeof rawContent === 'string') {
    return rawContent;
  }
  if (rawContent && typeof rawContent === 'object') {
    // Một số loại tin nhắn (share link, quoted...) có thể chứa text ở đây
    const candidate = rawContent.title || rawContent.description || rawContent.href || '';
...
export function startAntiLink(api) {
  api.listener.on("message", async (msg) => {
    ...
    const msgBody = getSafeMsgBody(data?.content);
